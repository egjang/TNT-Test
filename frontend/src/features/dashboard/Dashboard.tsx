...
export function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'compare' | 'arrears'>('compare')

  async function load(d: string) {
    setLoading(true)
    setError(null)
    try {
      const r = await fetch(`/api/v1/dashboard/sales-summary?date=${encodeURIComponent(d)}`)
      ...
      const [rPrev, rCur] = await Promise.all([
        fetch(`/api/v1/dashboard/monthly?year=${prevY}`),
        fetch(`/api/v1/dashboard/monthly?year=${y}&until=${encodeURIComponent(until)}`)
      ])
      ...
      const [rdPrev, rdCur] = await Promise.all([
        fetch(`/api/v1/dashboard/daily?year=${prevY}&month=${month}`),
        fetch(`/api/v1/dashboard/daily?year=${y}&month=${month}&until=${encodeURIComponent(until)}`)
      ])
