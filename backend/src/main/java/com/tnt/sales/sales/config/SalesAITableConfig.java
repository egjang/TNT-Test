package com.tnt.sales.sales.config;
...
/**
 * AI Agent 매출 분석 테이블 설정을 로드하는 컴포넌트
 * sales-ai-tables.yml 파일에서 테이블 정의를 읽어옵니다.
 */
@Component
public class SalesAITableConfig {
    private static final Logger log = LoggerFactory.getLogger(SalesAITableConfig.class);
    ...
    public void loadConfig() {
        try {
            ClassPathResource resource = new ClassPathResource("sales-ai-tables.yml");
            Yaml yaml = new Yaml();
            ...
            try (InputStream inputStream = resource.getInputStream()) {
                ...
                if (tableConfigs != null) {
                    for (Map<String, Object> tableConfig : tableConfigs) {
                        ...
                        if (table.isEnabled()) {
                            tables.add(table);
                            log.info("Loaded Sales AI table config: {}", table.getName());
                        }
                    }
                }

                log.info("Total {} Sales AI analysis tables loaded", tables.size());
            }
        } catch (Exception e) {
            log.error("Failed to load sales-ai-tables.yml", e);
        }
    }
