...
public class RAGController {
    ...
    private String buildSystemPromptWithDocuments(List<String> documentNames) {
        ...
        // 검색 정확도 향상 지시
        sb.append("검색 시 주의사항:\n");
        sb.append("- '영업관리단위' 검색 시: 문서에서 '영업관리단위:' 패턴으로 검색하여 모든 종류를 찾아 완전한 목록을 제공하세요\n");
        sb.append("- '품목 목록' 검색 시: 검색된 모든 품목을 빠짐없이 나열하세요\n");
        sb.append("- '분류' 검색 시: 대분류, 중분류, 소분류 계층 구조를 명확히 표시하세요\n");
        sb.append("- 검색 결과가 많으면 요약하지 말고 전체를 표시하세요\n\n");

        // HTML 형식 응답 지시
        sb.append("중요: 응답은 HTML 형식으로 작성하세요:\n");
        sb.append("- 목록 형태의 답변은 <table> 태그로 테이블을 만들어 표시하세요 (예: 품목 목록, 거래처 목록 등)\n");
        sb.append("- 테이블은 <table><thead><tr><th>헤더</th></tr></thead><tbody><tr><td>데이터</td></tr></tbody></table> 형식으로 작성\n");
        sb.append("- 중요한 내용은 <strong> 태그로 강조하세요\n");
        sb.append("- 순서가 있는 항목은 <ol><li> 태그 사용\n");
        sb.append("- 순서가 없는 항목은 <ul><li> 태그 사용\n");
        sb.append("- 단락 구분은 <p> 태그 사용\n");
        sb.append("- 제목이 필요하면 <h3> 또는 <h4> 태그 사용\n");
        sb.append("- 일반 텍스트도 읽기 쉽게 구조화하세요\n");
